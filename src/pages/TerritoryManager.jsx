        ...
        <div className="flex flex-col h-full gap-6">
            <header className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-slate-500 text-sm mt-1">Define jurisdiction boundaries for field agents — radius or custom polygon</p>
                </div>
                {successMsg && (
                ...
                <div className="bg-white border border-slate-200 rounded-3xl p-5 flex flex-col gap-3">
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-10 pr-4 py-2.5 text-sm text-slate-900 focus:border-primary-500 outline-none transition"
                        />
                    </div>
                    <div className="flex-1 overflow-y-auto space-y-2 max-h-[500px]">
                        {loading ? (
                            ...
                            filteredEmployees.map(emp => (
                                ...
                                >
                                    <span className="text-slate-900 font-semibold block text-sm">{emp.full_name}</span>
                ...
                <div className="lg:col-span-3 flex flex-col gap-6">
                    {selectedEmployee ? (
                        <>
                            ...
                            <div className="bg-white border border-slate-200 rounded-2xl p-5 flex items-center gap-4 flex-wrap">
                                <div className="flex-1 min-w-[200px]">
                                    ...
                                    <select
                                        ...
                                        className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-slate-900 text-sm focus:border-primary-500 outline-none"
                                    >
                                ...
                                {!isPolygonMode ? (
                                    ...
                                    <div className="flex items-end gap-2">
                                        ...
                                        <button
                                            ...
                                            className="px-4 py-2.5 rounded-xl text-sm font-medium bg-slate-100 text-slate-500 hover:text-slate-900 transition-all border border-slate-300"
                                        >
                                            <Trash2 size={14} />
                                        </button>
                            ...
                            {/* Live Map */}
                            <div className="flex-1 bg-white border border-slate-200 rounded-2xl overflow-hidden min-h-[450px]">
                                <MapContainer
                            ...
                            {isPolygonMode && polygonPoints.length > 0 && (
                                <div className="bg-white border border-slate-200 rounded-2xl p-4">
                                    <h3 className="text-sm font-bold text-slate-500 mb-2">Polygon Vertices ({polygonPoints.length})</h3>
                                    <div className="flex flex-wrap gap-2">
                                        {polygonPoints.map((p, i) => (
                                            <span key={i} className="bg-slate-100 text-slate-600 px-3 py-1.5 rounded-lg text-xs flex items-center gap-2 border border-slate-300">
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center text-center bg-white border border-slate-200 rounded-3xl">
                            <Target size={64} className="text-slate-800 mb-4" />
                            <h3 className="text-slate-500 text-lg">Select an agent to manage their territory</h3>
                            <p className="text-slate-600 max-w-sm mt-2">
                                Territories ensure field staff operate within their assigned sales jurisdictions.
                                Choose <strong className="text-slate-500">Circular</strong> for simple radius or <strong className="text-emerald-500">Polygon</strong> for precise boundaries.
                            </p>
                        </div>
                    )}
                </div>

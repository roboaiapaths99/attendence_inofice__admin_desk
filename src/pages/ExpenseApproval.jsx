        ...
        <div className="space-y-6 max-w-[1600px] mx-auto">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                <div>
                    ...
                    <p className="text-slate-500 text-sm mt-2 font-medium">Verify field expenditures and mobility reimbursements</p>
                </div>

                <div className="flex items-center gap-3 bg-white p-1.5 rounded-2xl border border-slate-200 backdrop-blur-md">
                    <button
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 py-4 border-y border-slate-200">
                    ...
                    {['pending', 'approved', 'rejected', 'queried', ''].map(status => (
                        <button
                            ...
                            className={`px-5 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all border ${statusFilter === status
                                ? 'bg-slate-100 text-primary-400 border-primary-500/30'
                                : 'text-slate-500 border-transparent hover:text-slate-600 hover:bg-slate-100'
                                }`}
                        >
                            {status || 'All Claims'}
                        </button>
                ...
                <div className="relative group">
                    ...
                    <input
                        ...
                        className="bg-white border border-slate-200 rounded-xl pl-10 pr-4 py-2 text-sm text-slate-900 outline-none focus:border-primary-500/50 transition-all w-64"
                    />
                </div>
            </div>
            ...
            {loading ? (
                ...
                <div className="bg-white border border-slate-200 border-dashed rounded-[2rem] p-24 text-center">
                    <div className="w-20 h-20 bg-white rounded-3xl flex items-center justify-center mx-auto mb-6 border border-slate-200">
                        <Receipt size={32} className="text-slate-700" />
                    </div>
                    <h3 className="text-xl font-bold text-slate-500">Quiet for now</h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-6">
                    {(activeTab === 'standard' ? claims : kmClaims).map(claim => (
                        <div key={claim._id} className="bg-white backdrop-blur-md border border-slate-200 rounded-[1.5rem] p-6 hover:border-primary-500/30 transition-all group flex flex-col h-full relative overflow-hidden">
                            {/* Status Stripe */}
                            <div className={`absolute left-0 top-0 bottom-0 w-1 ${statusFilter === 'pending' ? 'bg-amber-500/50' : 'bg-slate-100'}`} />

                            {/* Header */}
                            <div className="flex items-start justify-between mb-6">
                                <div className="flex gap-4">
                                    <div className="w-12 h-12 rounded-2xl bg-slate-100 flex items-center justify-center text-lg font-black text-slate-600 border border-slate-300 group-hover:bg-primary-500/10 group-hover:text-primary-400 transition-colors">
                                        {(claim.employee_name || claim.full_name || 'U').charAt(0)}
                                    </div>
                                    <div>
                                        <h3 className="text-slate-900 font-bold tracking-tight text-lg">{claim.employee_name || claim.full_name}</h3>
                                    ...
                                    <div className="bg-slate-50 rounded-2xl p-4 border border-slate-200">
                                        ...
                                        <div className="grid grid-cols-2 gap-4 pt-2 border-t border-slate-200">
                                            <div>
                                                ...
                                                <p className="text-sm font-bold text-slate-900">{claim.total_km} KM</p>
                                            </div>
                                            <div>
                                                <p className="text-[10px] text-slate-500 font-black uppercase tracking-tighter">Applied Rate</p>
                                                <p className="text-sm font-bold text-slate-900">₹{claim.rate_per_km}/km</p>
                                            </div>
                                        </div>
                                        <div className="mt-4 pt-4 border-t border-slate-200">
                                            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mb-1">Billing Date</p>
                                            <p className="text-sm text-slate-600 font-medium">{new Date(claim.date).toLocaleDateString(undefined, { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p>
                                        </div>
                                    </div>
                                ) : (
                                    <>
                                        <div className="flex justify-between items-center bg-slate-50 rounded-2xl p-4 border border-slate-200">
                                            <div>
                                                ...
                                                <p className="text-slate-600 text-sm italic line-clamp-2">"{claim.description || 'No description provided'}"</p>
                                            </div>
                                            <div className="text-right">
                                                <p className="text-[10px] text-slate-500 font-bold">Claimed</p>
                                                <p className="text-xl font-black text-slate-900">₹{claim.amount?.toLocaleString()}</p>
                                            </div>
                                        </div>
                                            ...
                                            <div className="grid grid-cols-1 gap-3 p-4 bg-primary-500/5 rounded-2xl border border-primary-500/10">
                                                ...
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-xs text-slate-800">{claim.location_city || 'Assigned Territory'}</p>
                                                    </div>
                                                </div>
                                            </div>
                                        )}
                                            ...
                                            <div className="p-4 bg-slate-50 border border-slate-200 rounded-2xl">
                                                ...
                                                <div className="grid grid-cols-2 gap-4">
                                                    <div>
                                                        ...
                                                        <p className="text-sm font-bold text-slate-900">{claim.claimed_km || 0}</p>
                                                    </div>
                                                    <div>
                                        ...
                                        {/* Receipt */}
                                        {claim.receipt_url && (
                                            <div className="relative group/img aspect-video rounded-2xl overflow-hidden border border-slate-200">
                                                <img
                                                ...
                                                <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-slate-950 to-transparent p-4 opacity-0 group-hover/img:opacity-100 transition-opacity">
                                                    <div className="flex items-center justify-between">
                                                        <span className="text-[10px] font-bold text-slate-900 uppercase tracking-widest shadow-lg">Tap to verify</span>
                                        ...
                                        <div className="mt-4 animate-in slide-in-from-top-2 duration-300">
                                            <div className="relative">
                                                <input
                                                    ...
                                                    className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm text-slate-900 placeholder:text-slate-600 outline-none focus:border-primary-500/50 transition-all pr-12"
                                                />
                                                <button
            ...
            {viewingReceipt && (
                <div
                    className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-50 p-10 backdrop-blur-sm cursor-zoom-out"
                    onClick={() => setViewingReceipt(null)}
                >
                    <div className="relative max-w-5xl w-full max-h-full flex flex-col gap-4">
                        <img
                            src={viewingReceipt}
                            className="w-full h-auto max-h-[85vh] object-contain rounded-2xl shadow-2xl border border-slate-200 shadow-primary-500/5 animate-in zoom-in-95 duration-300"
                            alt="Receipt Audit View"
                        />
                        <div className="flex justify-between items-center bg-white p-4 rounded-2xl border border-slate-200">
                            <p className="text-slate-500 text-xs font-bold uppercase tracking-widest">Document Integrity Checked</p>
                            <button className="text-slate-500 hover:text-slate-900 flex items-center gap-2 text-sm font-bold transition-colors">

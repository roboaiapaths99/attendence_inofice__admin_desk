        ...
        <div className="flex flex-col h-full gap-6">
            <header className="flex justify-between items-center bg-white p-6 rounded-3xl border border-slate-200">
                <div>
                    ...
                    <p className="text-slate-500 text-sm mt-1">Live Zomato-style tracking (Active Duty Only)</p>
                </div>
                <div className="flex gap-4">
                    <button
                        onClick={() => setShowHeatmap(!showHeatmap)}
                        className={`px-4 py-2 rounded-2xl border transition-all flex items-center gap-2 text-xs font-bold ${showHeatmap ? 'bg-amber-500/20 border-amber-500 text-amber-500 shadow-[0_0_15px_rgba(245,158,11,0.3)]' : 'bg-white border-slate-200 text-slate-500 hover:border-slate-300'}`}
                    >
                    ...
                    <div className="bg-slate-50 px-4 py-2 rounded-2xl border border-slate-200 flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <span className="text-xs font-bold text-slate-900">Duty Filter ON</span>
                        </div>
                    </div>
                </div>
            </header>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 flex-1 min-h-[600px]">
                {/* Agent List */}
                <div className="lg:col-span-1 bg-white border border-slate-200 rounded-3xl overflow-hidden flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto p-2 space-y-2">
                        {agents.map(agent => (
                            <button
                                ...
                                className={`w-full text-left p-4 rounded-2xl transition-all border ${selectedAgent?.id === agent.id ? 'bg-primary-500/10 border-primary-500/50 shadow-lg' : 'hover:bg-slate-100 border-transparent'}`}
                            >
                ...
                {/* Map View */}
                <div className="lg:col-span-3 bg-white border border-slate-200 rounded-3xl overflow-hidden relative shadow-2xl">
                    <MapContainer
                                    ...
                                    <div className="p-3 bg-white rounded-lg min-w-[200px]">
                                        ...
                                        <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-200">
                                            <div>
                                                ...
                                                <p className="text-sm font-bold text-slate-900">{agent.km_today || 0} KM</p>
                                            </div>
                                            <div>
                                                <p className="text-[8px] text-slate-500 uppercase font-bold">Last Active</p>
                                                <p className="text-sm font-bold text-slate-900">{formatToIST(agent.last_ping, { hour: '2-digit', minute: '2-digit' })}</p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="mt-4 pt-3 border-t border-slate-200 flex flex-col gap-2">
                                            <button
                                                ...
                                                className="w-full bg-[#004B87] hover:bg-[#004B87] text-slate-900 text-[10px] font-bold py-2 rounded-lg flex items-center justify-center gap-2"
                                            >
                    ...
                    <div className="absolute bottom-6 left-6 z-[1000] bg-slate-50 backdrop-blur-md px-4 py-2 rounded-xl border border-slate-200 text-[10px] text-slate-500 flex items-center gap-2">

...
const ROLE_ICONS = {
    ...
    manager: { icon: Network, color: 'text-[#004B87]', bg: 'bg-[#004B87]/10', border: 'border-[#004B87]/30' }
};
...
const OrgStructure = () => {
    ...
    return (
        <div className="space-y-10 pb-20">
            <div>
                <h1 className="text-2xl font-bold text-slate-900">Organization Map</h1>
                <p className="text-slate-500 text-sm">Visual hierarchy and reporting structure</p>
            </div>

            {/* Level 1: Owner */}
            <section className="flex flex-col items-center">
                <div className="px-4 py-2 bg-slate-100 border border-slate-300 rounded-lg text-[10px] text-slate-500 uppercase tracking-widest font-black mb-4">
                    Organizational Head
                </div>
                <div className="flex flex-wrap justify-center gap-6">
                    {owners.map(owner => (
                        <div key={owner.email} className="bg-white border-2 border-amber-500/30 p-6 rounded-[2rem] w-72 shadow-2xl shadow-amber-900/10 relative group">
                            ...
                            <div className="flex flex-col items-center text-center">
                                ...
                                <h3 className="text-slate-900 font-bold text-lg">{owner.full_name}</h3>
            ...
            <section className="flex flex-col items-center">
                ...
                <div className="flex flex-wrap justify-center gap-6">
                    {hrSupport.map(admin => {
                        ...
                        return (
                            <div key={admin.email} className={`bg-white border border-slate-200 p-5 rounded-3xl w-64 group hover:border-slate-300 transition-all`}>
                                <div className="flex items-center gap-4">
                                    <div className={`w-12 h-12 ${config.bg} rounded-xl flex items-center justify-center border ${config.border}`}>
                                        <Icon className={config.color} size={24} />
                                    </div>
                                    <div>
                                        <h4 className="text-slate-900 font-semibold text-sm">{admin.full_name}</h4>
            ...
            {/* Connector Line */}
            <div className="h-8 w-1 bg-gradient-to-b from-purple-500/30 to-[#004B87]/30 mx-auto" />

            {/* Level 3: Managers and their Teams */}
            <section className="flex flex-col items-center">
                <div className="px-4 py-2 bg-slate-100 border border-slate-300 rounded-lg text-[10px] text-slate-500 uppercase tracking-widest font-black mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 w-full">
                        ...
                        return (
                            <div key={manager.email} className="flex flex-col items-center">
                                ...
                                <div
                                    ...
                                    className={`bg-white border ${isExpanded ? 'border-primary-500/50' : 'border-slate-200'} p-6 rounded-3xl w-full cursor-pointer hover:bg-slate-100 transition-all group relative`}
                                >
                                    <div className="flex items-center justify-between mb-4">
                                        <div className="w-12 h-12 bg-[#004B87]/10 rounded-2xl flex items-center justify-center border border-[#004B87]/30">
                                            <Network className="text-[#0062B1]" size={24} />
                                        </div>
                                        <div className="bg-slate-50 px-3 py-1 rounded-full border border-slate-200 text-[10px] font-bold text-slate-500">
                                            {team.length} Members
                                        </div>
                                    </div>
                                    <h4 className="text-slate-900 font-bold">{manager.full_name}</h4>
                                ...
                                <AnimatePresence>
                                    {isExpanded && team.length > 0 && (
                                        ...
                                        >
                                            {team.map(member => (
                                                <div key={member.email} className="flex items-center gap-3 p-3 bg-white border border-slate-200 rounded-2xl">
                                                    ...
                                                    <div className="min-w-0">
                                                        <p className="text-xs font-bold text-slate-900 truncate">{member.full_name}</p>
                    ...
                    {unassignedEmployees.length > 0 && (
                        <div className="flex flex-col items-center">
                            <div className="bg-white border border-red-500/20 p-6 rounded-3xl w-full border-dashed">
                                <div className="flex items-center justify-between mb-4">
                                    <div className="w-12 h-12 bg-red-500/10 rounded-2xl flex items-center justify-center border border-red-500/20">
                                        <Users className="text-red-400" size={24} />
                                    </div>
                                    <div className="bg-slate-50 px-3 py-1 rounded-full border border-slate-200 text-[10px] font-bold text-red-400">
                                        {unassignedEmployees.length} Pending
                                    </div>
                                </div>
                                <h4 className="text-slate-900 font-bold">Unmapped Staff</h4>

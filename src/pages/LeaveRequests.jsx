                ...
                <div className="flex bg-white p-1 rounded-xl border border-slate-200">
                    {['pending', 'approved', 'rejected', 'cancelled'].map((s) => (
                        <button
                            key={s}
                            onClick={() => setFilter(s)}
                            className={`px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-widest transition-all ${filter === s ? 'bg-primary-600 text-white' : 'text-slate-500 hover:text-slate-600'
                                }`}
                        >
                            {s}
                        </button>

            ...
            >
                ...
                <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-gradient-to-r from-blue-50 to-white">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-xl font-bold text-slate-900">{data?.full_name || employee?.full_name}</h2>
                    ...
                    <div className="flex items-center gap-3">
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="pl-10 pr-4 py-2 bg-white border border-slate-200 rounded-xl text-sm font-medium focus:ring-2 focus:ring-blue-500 outline-none transition-all"
                            />
                        </div>
                        <button
                            onClick={onClose}
                                ...
                                <table className="w-full text-left border-collapse">
                                    ...
                                    <tbody className="divide-y divide-slate-100">
                                        {(data.daily_breakdown || []).map((day) => (
                                            <tr
                                                key={day.date}
                                                className={`hover:bg-blue-50/50:bg-blue-900/10 transition-colors group cursor-pointer ${selectedDay?.date === day.date ? 'bg-blue-50/50' : ''}`}
            ...
            <AnimatePresence>
                {selectedDay && (
                    <motion.div
                        ...
                        className="fixed right-0 top-0 bottom-0 w-full max-w-md bg-white shadow-2xl border-l border-slate-100 z-[60] flex flex-col"
                    >
                        <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                            <div>
                                <h3 className="font-bold text-lg text-slate-900">Day Details</h3>
                                <p className="text-sm text-slate-500">{selectedDay.date}</p>
                            </div>
                            <button onClick={() => setSelectedDay(null)} className="p-2 hover:bg-slate-200:bg-slate-700 rounded-full transition-colors font-bold">
                                <X size={20} />
                            </button>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-6 space-y-6">
                            {selectedDay.logs.length > 0 ? selectedDay.logs.map((log, idx) => (
                                <div key={idx} className="p-5 rounded-2xl bg-white border border-slate-100 space-y-4 shadow-sm">
                                    <div className="flex justify-between items-start">
                                        <div className="flex items-center gap-3">
                                            <div className={`p-2.5 rounded-xl ${log.type === 'check-in' ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
                                                {log.type === 'check-in' ? <Fingerprint size={18} /> : <X size={18} />}
                                            </div>
                                            <div>
                                                <p className="font-bold capitalize text-slate-900">{log.type.replace('-', ' ')}</p>
                                    ...
                                    {log.location && (
                                        <div className="p-3 bg-slate-50 rounded-xl flex items-center justify-between group">
                                    ...
                                    <div className="flex items-center gap-6 pt-2 border-t border-slate-50">
                                        {log.wifi_confidence > 0 && (
                                            <div className="flex items-center gap-1.5 text-xs text-slate-500">
                                                ...
                                                <span className="font-medium text-slate-700">{log.wifi_confidence}% WiFi</span>
                                            </div>
                                        )}
                                        <div className="flex items-center gap-1.5 text-xs text-slate-500">
                                            <Clock size={14} className="text-slate-400" />
                                            <span className="font-medium text-slate-700">{log.check_in_method || 'Manual'}</span>
                                        </div>
                                    </div>
                                </div>
                            )) : (
...
const StatusBadge = ({ status }) => {
    ...
    const styles = {
        ...
        "Early Leave": "bg-yellow-100 text-yellow-700",
...
const LocationDisplay = ({ lat, lon }) => {
    ...
    if (loading) return <span className="animate-pulse bg-slate-200 h-4 w-32 rounded"></span>;
